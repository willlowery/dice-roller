...
public class CondOperator extends SExpression.Lambda {
    ...
    public SExpression eval(List<SExpression> rest, Interpreter interpreter, SContext definitions) {
        if (rest.isEmpty()) {
            return new SExpression.Error("cond requires at least one clause");
        }

        for (SExpression clause : rest) {
            if (!(clause instanceof SExpression.SList pair) || pair.value().size() != 2) {
                return new SExpression.Error("each cond clause must be a list of (condition expression)");
            }
        ...
        return new SExpression.Error("no matching cond clause");
    }
}
